...
public class JTetris extends JComponent {
	...
	JTetris(int pixels) {
		...
		*/
		...
		timer = new javax.swing.Timer(DELAY, new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				tick(DOWN); // listens for event, tick(DOWN) called when timer event occurs 
			}
		});
		
		requestFocusInWindow(); // gives window focus first time window is activated
	}
	...
	*/
	public void startGame() {
		...
		if (testMode) random = new Random(0);	// same seq every time, 0 = seed
